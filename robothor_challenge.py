...
ALLOWED_ACTIONS = ["MoveAhead", "RotateLeft", "RotateRight", "Stop", "LookUp", "LookDown"]
...
def distance_from_point_to_object_type(
    ...
    if path:
        ...
        s_dist = math.sqrt((point["x"] - path[0]["x"]) ** 2 + (point["z"] - path[0]["z"]) ** 2)
...
class RobothorChallenge:
    def __init__(
        ...
        if env_type == EnvTypes.ROBOTHOR:
            ...
            self.controller_kwargs["local_executable_path"] = (
                "./pasture_builds/thor_build_normal/normal.x86_64"
            )
        elif env_type == EnvTypes.DUP:
            self.controller_kwargs["local_executable_path"] = (
                "./pasture_builds/thor_build_dup/dup.x86_64"
            )
        elif env_type == EnvTypes.REMOVE:
            self.controller_kwargs["local_executable_path"] = (
                "./pasture_builds/thor_build_remove/remove.x86_64"
            )
        elif env_type == EnvTypes.LONGTAIL:
            self.controller_kwargs["local_executable_path"] = (
                "./pasture_builds/thor_build_longtail/longtail.x86_64"
            )
        else:
    ...
    def inference_worker(
        ...
        assert controller.step("SetRandomSeed", seed=seed).metadata["lastActionSuccess"]
        ...
        while True:
            ...
            logger.info(
                "Task Start id:{id} scene:{scene} target_object:{object_type}"
                " initial_position:{initial_position} rotation:{initial_orientation}".format(**e)
            )
            controller.initialization_parameters["robothorChallengeEpisodeId"] = e["id"]
            print(e["scene"], e["object_type"])
            controller.reset(e["scene"])
            event = controller.step("SetRandomSeed", seed=seed)
            ...
            while total_steps < max_steps and stopped is False:
                if env_type == EnvTypes.ROBOTHOR or env_type == EnvTypes.LONGTAIL:
                    ...
                    target_obj = get_object_by_id(event.metadata["objects"], e["object_id"])
                ...
                if (
                    "renderInstanceSegmentation" in controller_kwargs
                    and controller_kwargs["renderInstanceSegmentation"]
                ):
                    if env_type == EnvTypes.ROBOTHOR or env_type == EnvTypes.LONGTAIL:
                        observations["target_mask"] = query_instance_segmentation_for_object(
                            event.instance_masks, e["object_type"]
                        )
                    else:
                        observations["target_mask"] = query_instance_segmentation_for_object_id(
                            event.instance_masks, e["object_id"]
                        )
                ...
                episode_metrics["actions_taken"].append(
                    {"action": action, "success": event.metadata["lastActionSuccess"]}
                )
                stopped = action == "Stop"
    ...
    def inference(self, episodes, nprocesses=1, test=False):
        ...
        while len(metrics["episodes"]) < expected_count:
            try:
                ep_id, episode_metrics, episode_result = receive_queue.get(timeout=1000)
        ...
        if len(metrics["episodes"]):
            metrics["ep_len"] = sum(
                [len(em["trajectory"]) for em in metrics["episodes"].values()]
            ) / len(metrics["episodes"])

            if not test:
                metrics["success"] = sum([r["success"] for r in episode_results]) / len(
                    episode_results
                )
            ...
            if not test:
                logger.info(
                    "Total Episodes: {episode_count} Success:{success} SPL:{spl} Episode"
                    " Length:{ep_len}".format(
                ...
                logger.info(
                    "Total Episodes: {episode_count} Episode Length:{ep_len}".format(
    ...
    def _change_scene(self, scene, seed):
        if self.current_scene != scene:
            ...
            assert self.controller.step("SetRandomSeed", seed=seed).metadata["lastActionSuccess"]
    ...
    def move_to_point(self, datapoint, seed):
        ...
        logger.info(
            "Moving to position: {p}, y-rotation: {rot}, horizon: {hor}".format(
